...
function IncludeFile()              --导入其他模块
    ...
    dofile(CONFIG.ScriptPath .. "face_name.lua");
end
...
function Game_MMap()      --主地图
    ...
    local direct = -1;
    local keypress = lib.GetKeyPress();
    if keypress ~= -1 then
	    JY.MyTick=0;
    ...
    if JY.WalkDir ~= -1 then   --按下了光标键
        local time = lib.GetTime();
        if time - JY.LastWalkTime > CC.WalkSpeed then
            AddMyCurrentPic();         --增加主角贴图编号，产生走路效果
            x=JY.Base["人X"]+CC.DirectX[JY.WalkDir+1];
            y=JY.Base["人Y"]+CC.DirectY[JY.WalkDir+1];
            JY.Base["人方向"]=JY.WalkDir;
            JY.LastWalkTime = time;
        else
            x=JY.Base["人X"];
            y=JY.Base["人Y"];
        end
...
function Game_SMap()         --场景处理主函数
    ...
    if JY.WalkDir ~= -1 then
        local time = lib.GetTime();
        if time - JY.LastWalkTime > CC.WalkSpeed then
            AddMyCurrentPic();
            x=JY.Base["人X1"]+CC.DirectX[JY.WalkDir+1];
            y=JY.Base["人Y1"]+CC.DirectY[JY.WalkDir+1];
            JY.Base["人方向"]=JY.WalkDir;
            JY.LastWalkTime = time;
        else
            x=JY.Base["人X1"];
...
function TalkEx(s,headid,flag)
    return TalkEx(s, headid, flag, nil);
end

--复杂版本对话
--s 字符串，必须加上*作为分行，如果里面没有*,则会自动加上
function TalkEx(s,headid,flag, name)          --复杂版本对话
    ...
    while true do
        if dy==0 then
            ...
            if headid>=0 then
				...
				if name ~= nil then
				    DrawString(xy[flag].headx + 20, xy[flag].heady + boxpich, name,C_WHITE,CC.DefaultFont);
				end
            end
...
function instruct_1(talkid,headid,flag)        --对话
    ...
    local name = face_name[headid]
	if s==nil then        --对话id不存在
	    return ;
	end
    TalkEx(s,headid,flag, name);
end
...
function WarMain(warid,isexp)           --战斗主函数
    ...
    while true do             --战斗主循环
        if iWaitC>0 and first~=0 then                       --等待循环(鬼鬼标)
	    local iWaitD=iWaitC;
	    local iWaitE=iWaitC;
            for i=iWaitE,1,-1 do       
	        lib.Debug(sWarP[i]);
                if WAR.Person[sWarP[i]]["死亡"]==false then
                WAR.CurID=sWarP[i];
                local iR;
                if WAR.Person[sWarP[i]]["我方"]==true then
                    if WAR.AutoFight==0 then
                        repeat
                            sWait[iWaitD] = -1;
                            iR=War_Manual();                  --手动战斗
                        until sWait[iWaitD] == -1
	                iWaitC = iWaitD-1;
			iWaitD =iWaitD-1;
...
function War_AddPersonLevel(pid)      --人物是否升级
    ...
    if JY.Person[pid]["用毒能力"]>=20 then
        AddPersonAttrib(pid,"用毒能力",  Rnd(3)*leveladd);
    end
    if JY.Person[pid]["解毒能力"]>=20 then
        AddPersonAttrib(pid,"解毒能力",  Rnd(3)*leveladd);
    end
    if JY.Person[pid]["拳掌功夫"]>=20 then
        AddPersonAttrib(pid,"拳掌功夫",  Rnd(3)*leveladd);
    end
    if JY.Person[pid]["御剑能力"]>=20 then
        AddPersonAttrib(pid,"御剑能力",  Rnd(3)*leveladd);
    end
    if JY.Person[pid]["耍刀技巧"]>=20 then
        AddPersonAttrib(pid,"耍刀技巧",  Rnd(3)*leveladd);
    end
    if JY.Person[pid]["暗器技巧"]>=20 then
        AddPersonAttrib(pid,"暗器技巧",  Rnd(3)*leveladd);
    end
...
function War_WaitMenu()            --等待，把当前战斗人调到队尾
    iWaitC = iWaitC+1;
    sWait[iWaitC] = WAR.Person[WAR.CurID]["人物编号"];
    sWarP[iWaitC] = WAR.CurID;
    lib.Debug(sWarP[iWaitC]);
    return 1;
